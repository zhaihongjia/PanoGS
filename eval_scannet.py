...
import datasets.scannet_class_utils as scannet_class_utils
from utils.eval_utils import calculate_iou_3d

def panoptic_evaluate_scannet_scene(pred_semantics, pred_instances, gt_semantics, gt_instances, thing=False):
    metrics = {}
    thing_mask = scannet_class_utils.SCANNET_THING_MASK_21
    ...
    for i in range(len(thing_mask)):
        ...
        # use i-1 is due to unlable is not in SCANNET_LABELS_20
        label_name = scannet_class_utils.SCANNET_LABELS_20[i-1]
...
def evaluate_scannet(args):
    ...
    for scene in scenes:
        ...
        gt_sem = np.load(f'{args.gt_path}/{scene}/semantic_labels.npy')
        gt_ins = np.load(f'{args.gt_path}/{scene}/instance_labels.npy')
        ...
        # eval Thing
        metrics = panoptic_evaluate_scannet_scene(pre_sem, pre_ins, gt_sem, gt_ins, thing=True)
        ...
        # eval Stuff
        metrics = panoptic_evaluate_scannet_scene(pre_sem, pre_ins, gt_sem, gt_ins, thing=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--gt_path", type=str, default='./datasets/scannet/3d_sem_ins')
    args = parser.parse_args()

    evaluate_scannet(args)
